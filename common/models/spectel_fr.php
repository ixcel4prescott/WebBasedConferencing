<?
class SpectelFr extends AppModel
{
  var $useTable     = false;
  var $useDbConfig  = 'spectel_fr';
  ...
  return false;
    ...
    if($conference = $this->getConferenceByCode($room['cec'], $room['pec'])) {
      $this->error_msg = sprintf('Conference A%s exists with given (CEC,PEC) = (%s,%s)', $conference, $room['cec'], $room['pec']);
      return false;
    }
  ...
  function getConference($accountid)
  {
    ...
    if(substr($accountid, 0, 2) == 'FR')
      $confirm_num = substr($accountid, 2);
  ...
  function updateRoom($room, $data) 
  {
    ...
    if(substr($room['Room']['accountid'], 0, 2) == 'FR')
      $confirm_num = substr($room['Room']['accountid'], 2);
