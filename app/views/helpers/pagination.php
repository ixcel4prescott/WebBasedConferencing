...
class PaginationHelper extends Helper 
{
	...
	if(is_array($pageDetails['resultsPerPage']))
	  {
		...
		if($OriginalValue == $value)
		  {
		    $t .= '<span class="current">'.$value.'</span>'.$separator;
		  }
		else
		  {
  ...
  {
    if (empty($this->_pageDetails) || $this->_pageDetails['pageCount'] == 1) { return '<span class="current">1</span>'; }
  ...
  function sortBy ($value, $title=NULL, $Model=NULL,$escapeTitle=false,$upText=null,$downText=null) 
  {
    ...
    if(!$upText)
      $upText = $this->Html->image('up.gif');
    
    if(!$downText)
      $downText = $this->Html->image('down.gif');
