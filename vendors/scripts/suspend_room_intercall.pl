...
#--------------------------------------------------------------------------------
#
# suspend_room_intercall.pl
#  Disable a room via Intercall's SOAP webservice
# 
...
sub do_request {
	...
	->autotype(0)
	->on_debug(\&debug)
	->call(SOAP::Data->name('stop-activity-owner-request')->attr({'xmlns' => NS, 'action' => 'disable'}) => 
...
sub mark_as_suspended {
    my ($accountid) = @_;
    return $ic->do("UPDATE account SET roomstat=1,roomstatdate=CURRENT_TIMESTAMP WHERE accountid=?", undef, $accountid);
}
...
sub main {
	...
	mark_as_suspended($opts{'c'})
	    or die sprintf("Could not mark %s room as suspended", $opts{'c'});

	printf("Room suspended: %s\n", $opts{'c'});
